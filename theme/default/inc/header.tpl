...
<div id="mainNav">
 <ul class="wrap">
  ...
  <li{if $nav.cur} class="cur hover"{/if}><a href="{$nav.url}"{if $smarty.foreach.nav_middle_list.iteration eq 4} class="last"{/if}{if $nav.target} target="_blank"{/if}>{$nav.nav_name}</a>
  <!-- {if $nav.child} -->
  <ul>
   <!-- {foreach from=$nav.child item=child} -->
